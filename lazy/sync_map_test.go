...
func TestSyncMapInit(t *testing.T) {
	var sm SyncMap[string, int]
	if !sm.Init("42", 42) {
...
func TestSyncMapInitAfterDelete(t *testing.T) {
	var sm SyncMap[string, int]
	if !sm.Init("42", 42) {
		t.Fatal("Init failed")
	}
	sm.Delete("42")
	if !sm.Init("42", 43) {
		t.Fatal("Init after Delete failed")
	}
...
func TestSyncMapReplace(t *testing.T) {
	...
	sm.Replace("42", 43)
	if sm.Init("42", 44) {
		t.Fatal("Init succeeded after Replace")
	}
	if got := sm.Get("42", func() int {
...
func TestSyncMapRefillDoesNotBlockOtherKeys(t *testing.T) {
	var (
		sm      SyncMap[string, int]
		filling = make(chan struct{})
		release = make(chan struct{})
	...
	if got := sm.Get("other", func() int { return 100 }); got != 100 {
		t.Fatalf("Get(other) = %v; want 100", got)
	}
	close(release)
	if got := <-done; got != 43 {
		t.Fatalf("Refill = %v; want 43", got)
